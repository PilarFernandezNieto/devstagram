...
@section('contenido')
    <div class="flex justify-center">
        <div class="w-full md:w-8/12 lg:w-6/12 flex flex-col items-center md:flex-row">
            ...
            <div class="md:w-8/12 lg:w-6/12 px-5 flex flex-col items-center py-10 md:items-start md:justify-center">
                ...
                @auth
                    {{-- oculta el formulario para el usuario que está autenticado / evita que se siga a sí mismo --}}
                    @if ($user->id != auth()->user()->id)
    ...
    <section class="container mx-auto mt-10">
                    ...
                    <div>
                        <a href="{{ route('posts.show', ['user' => $user, 'post' => $post]) }}">
                            <img src="{{ asset('uploads') . '/' . $post->imagen }}"
                                alt="Imagen del post {{ $post->titulo }}" class="rounded">
                        </a>
                    </div>
                @endforeach
            </div>
        ...
        @else
            <p class="text-gray-600 uppercase text-sm text-center font-bold">Todavía no hay publicaciones</p>
        @endif
    </section>
@endsection
